...
CUMATRIX compute_hog_features(CUMATRIX B, int sbin)
{
  ...
  dims[2] = 18;
  H_device.dims_num = 3;
  error = cudaMalloc((void**)&(H_device.dims), sizeof(int)*3);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc H_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemcpy(H_device.dims, dims, sizeof(int)*3, cudaMemcpyHostToDevice);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy H_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  H_device.length = dims[0]*dims[1]*dims[2];
  error = cudaMalloc((void**)&(H_device.data), sizeof(float)*H_device.length);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc H_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemset(H_device.data, 0, sizeof(float)*H_device.length);
  if (error != cudaSuccess)
  {
    printf("cudaMemset H_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  N_device.dims_num = 2;
  error = cudaMalloc((void**)&(N_device.dims), sizeof(int)*2);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc N_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemcpy(N_device.dims, blocks, sizeof(int)*2, cudaMemcpyHostToDevice);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy N_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  N_device.length = blocks[0]*blocks[1];
  error = cudaMalloc((void**)&(N_device.data), sizeof(float)*N_device.length);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc N_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemset(N_device.data, 0, sizeof(float)*N_device.length);
  if (error != cudaSuccess)
  {
    printf("cudaMemset N_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
  ...
  error = cudaMemset(HOG_device.data, 0, sizeof(float)*HOG_device.length);
  if (error != cudaSuccess)
  {
    printf("cudaMemset HOG_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  // copy to constant memory
  error = cudaMemcpyToSymbol(uu_dev, uu, sizeof(float)*9);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpyToSymbol uu_dev returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemcpyToSymbol(vv_dev, vv, sizeof(float)*9);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpyToSymbol vv_dev returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
  ...
  error = cudaMemcpy(HOG.data, HOG_device.data, sizeof(float)*HOG.length, cudaMemcpyDeviceToHost);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy HOG.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
