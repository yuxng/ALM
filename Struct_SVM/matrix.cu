...
CUMATRIX alloc_device_cumatrix(CUMATRIX mat_host)
{
  ...
  error = cudaMalloc((void**)&(mat_device.dims), sizeof(int)*dims_num);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc mat_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  mat_device.length = length;
  error = cudaMalloc((void**)&(mat_device.data), sizeof(float)*length);
  if (error != cudaSuccess)
  {
    printf("cudaMalloc mat_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  /* copy host memory to device */
  error = cudaMemcpy(mat_device.dims, mat_host.dims, sizeof(int)*dims_num, cudaMemcpyHostToDevice);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy mat_device.dims returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemcpy(mat_device.data, mat_host.data, sizeof(float)*length, cudaMemcpyHostToDevice);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy mat_device.data returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
