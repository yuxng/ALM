...
CUMATRIX rectify_image(CUMATRIX A, float *H, float *T)
{
  ...
  error = cudaMemcpyToSymbol(X_device, X, sizeof(float)*4);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpyToSymbol X_device returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }

  error = cudaMemcpyToSymbol(Y_device, Y, sizeof(float)*4);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpyToSymbol Y_device returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
  ...
  error = cudaMemcpy(B.data, B_device.data, sizeof(float)*B.length, cudaMemcpyDeviceToHost);
  if (error != cudaSuccess)
  {
    printf("cudaMemcpy B returned error code %d, line(%d)\n", error, __LINE__);
    exit(EXIT_FAILURE);
  }
